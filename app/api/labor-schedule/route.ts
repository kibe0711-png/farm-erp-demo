...
export async function GET(request: Request) {
  try {
    ...
    const entries = await prisma.laborSchedule.findMany({
      where: {
    ...
    console.error("Failed to fetch labor schedule:", error);
...
export async function POST(request: Request) {
  try {
    ...
    // Insert new entries
    if (entries.length > 0) {
      await prisma.laborSchedule.createMany({
        data: entries.map((e) => ({
          farmPhaseId: e.farmPhaseId,
          laborSopId: e.laborSopId,
          weekStartDate: weekDate,
    ...
    console.error("Failed to save labor schedule:", error);
